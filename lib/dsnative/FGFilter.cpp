...
CFGFilter::CFGFilter(const CLSID& clsid, Type type, CStdStringA name)
  : m_clsid(clsid)
  , m_name(name)
  , m_type(type)
{
...
CFGFilterRegistry::CFGFilterRegistry(CStdStringA DisplayName) 
  : CFGFilter(GUID_NULL, CFGFilter::REGISTRY, L"")
  ...
  : CFGFilter(clsid, CFGFilter::REGISTRY, L"")
{
  ...
  CStdStringA guid = StringFromGUID(m_clsid);
...
HRESULT CFGFilterRegistry::Create(IBaseFilter** ppBF)
{
  ...
  {
    LoadRegistryFilter(m_clsid,ppBF);
...
CFGFilterFile::CFGFilterFile(const CLSID& clsid, CStdStringA path, CStdStringW name, CStdStringA internalName, CStdStringA filetype)
