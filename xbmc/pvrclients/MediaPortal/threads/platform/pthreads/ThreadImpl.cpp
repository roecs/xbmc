...
void CThread::Create(bool bAutoDelete, unsigned stacksize)
{
  if (m_ThreadId != 0)
  {
    logger->Log(LOGERROR, "%s - fatal error creating thread- old thread id not null", __FUNCTION__);
    exit(1);
  }
  ...
  {
    logger->Log(LOGNOTICE, "%s - fatal error creating thread",__FUNCTION__);
  }
...
void CThread::SetThreadInfo()
{
  ...
  if (setpriority(PRIO_PROCESS, m_ThreadOpaque.LwpId, appNice) != 0)
    logger->Log(LOGERROR, "%s: error %s", __FUNCTION__, strerror(errno));
}
