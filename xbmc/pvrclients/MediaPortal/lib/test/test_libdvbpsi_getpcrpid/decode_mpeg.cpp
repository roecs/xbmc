 ...
 *****************************************************************************/
#ifdef _MSC_VER
static int ReadPacket( HANDLE i_fd, uint8_t* p_dst )
{
    int i = 187;
    DWORD i_rc = 1;
    ...
    {
        ReadFile( i_fd, p_dst, 1, &i_rc, NULL);
    }

    while((i != 0) && (i_rc > 0))
    {
        ReadFile( i_fd, p_dst + 188 - i, i, &i_rc, NULL);
        if(i_rc >= 0)
            i -= i_rc;
    }
...
//    }
    ...
    if( filename )
    {
      if (( i_fd = ::CreateFile(filename,      // The filename
