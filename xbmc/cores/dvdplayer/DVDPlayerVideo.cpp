...
#ifdef HAS_DX  if(pSource->format == DVDVideoPicture::FMT_DXVA)    g_renderManager.AddProcessor(pSource->proc, pSource->proc_id, pSource->iFlags);  else if(pSource->format == DVDVideoPicture::FMT_DSHOW)    g_renderManager.AddProcessor(pSource->pAlloc, pSource->pSurfaceIndex);#endif}
#endif
